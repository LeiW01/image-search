...
def test_default_settings_are_local_and_pinned(tmp_path: Path) -> None:
    ...
    assert settings.fashion_weights_path == (
        tmp_path / "state" / "models" / "marqo-fashionSigLIP" / "open_clip_model.safetensors"
    )
    assert settings.dino_local_dir == tmp_path / "state" / "models" / "dinov2-base"
